...
#include <simd/simd.h>
#include "shader_types/S_thick_line_typesOLD.h"
#include "shader_types/uniform_types.h"

// ================ Trianglize Line ================
kernel void
INT_ThickLine_computeShaderOLD(              uint              tidx       [[thread_position_in_grid]],
                                   device INT_Cluster_T*    cluster    [[buffer(INT_triang_cluster_I)]],
                             const device ThickLinePoint_T* vertices   [[buffer(INT_vertices_I)]],
                                 constant UN_FrameDataOLD_T*   frame_data [[buffer(INT_aspect_ratio_I)]])
{
...
vertex RasterType
ThickLine_vertexShaderOLD(             uint            vidx       [[vertex_id]],
                       const device INT_Point_T*    vertices   [[buffer(vertices_I)]],
                           constant UN_FrameDataOLD_T* frame_data [[buffer(aspect_ratio_I)]])
{
    RasterType out;
...
fragment float4 ThickLine_fragmentShaderOLD(RasterType in [[stage_in]]) {
    in.uv *= in.uv;
